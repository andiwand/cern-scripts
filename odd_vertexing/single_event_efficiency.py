...
data = {
    VertexFinder.Iterative: {"event_vertices": [], "reco_vertices": []},
    VertexFinder.AMVF: {"event_vertices": [], "reco_vertices": []},
}
...
for vertexing in [VertexFinder.Iterative, VertexFinder.AMVF]:
  ...
  """
    ...
    for _, (x, y, z, px, py, pz) in particles_data[
        ["vx", "vy", "vz", "px", "py", "pz"]
    ].iterrows():
...
for vertexing in [VertexFinder.Iterative, VertexFinder.AMVF]:
    ...
    ax1.scatter(vertices[:, 2], vertices[:, 1], label=vertexing_label[vertexing])
