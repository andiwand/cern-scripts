...
def update(_):
    ...
    dv = np.array(
        [
            np.cos(phi.val) * np.sin(theta.val),
            np.sin(phi.val) * np.sin(theta.val),
            np.cos(theta.val),
        ]
    )
    ...
    set_data(
        direction_xy,
        np.array([dir_center, dir_center])
        + np.array(
            [[-0.25 * dv[0], -0.25 * dv[1], -0.25], [0.25 * dv[0], 0.25 * dv[1], -0.25]]
        ),
    )
    set_data(
        direction_xz,
        np.array([dir_center, dir_center])
        + np.array(
            [[-0.25 * dv[0], 0.25, -0.25 * dv[2]], [0.25 * dv[0], 0.25, 0.25 * dv[2]]]
        ),
    )
    set_data(
        direction_yz,
        np.array([dir_center, dir_center])
        + np.array(
            [[-0.25, -0.25 * dv[1], -0.25 * dv[2]], [-0.25, 0.25 * dv[1], 0.25 * dv[2]]]
        ),
    )
