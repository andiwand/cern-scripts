...
Reco_tf.py \
  --preExec "flags.Common.MsgSuppression=False; \
	     ...
	     flags.Acts.doLowPt=True; \
             from ActsConfig.ActsConfigFlags import SeedingStrategy;flags.Acts.SeedingStrategy=SeedingStrategy.GridTriplet;" \
  --preInclude "InDetConfig.ConfigurationHelpers.OnlyTrackingPreInclude,ActsConfig.ActsCIFlags.actsLegacyWorkflowFlags" \
  --postExec "cfg.getEventAlgo('ActsLegacyPixelSeedingAlg').OutputLevel=1" \
