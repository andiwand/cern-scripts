...
axs1.scatter(hits[barrel_mask & missing_mask]['tx'], hits[barrel_mask & missing_mask]['ty'], s=3)
axs2.scatter(hits['tz'].values, np.linalg.norm(hits[['tx', 'ty']], axis=-1), s=3)
axs2.scatter(hits[missing_mask]['tz'].values, np.linalg.norm(hits[missing_mask][['tx', 'ty']], axis=-1), s=3)
plt.show()
