#!/usr/bin/env python3
"""Score the shipped detectors against a reference, without refitting them.

`fit_event_config.py` prints this at the end of a fit, but a fit takes an hour
and the configuration it produced is already in `Fatras/data`. This scores what
is shipped, which is also what `validate.py` plots.
...
def main() -> None:
    ...
    config = presets.copy_with(config, overrides)
    cards = [fit.scorecard(presets.copy_with(config, {"seed": 9973 + i}),
                           layout, target)
             for i in range(5)]
